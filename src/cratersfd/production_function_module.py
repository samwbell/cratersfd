...
def loglog2linear_pf(pf):
    def linear_pf(d):
        return num1(10**pf(np.log10(d)))
    return linear_pf


def cumulative2differential_pf(
    c_pf, dmin=0.005, dmax=10**3.51, n_points=10000
):
    D = np.logspace(np.log10(dmin), np.log10(dmax), n_points)
    differential_array = -1 * np.gradient(c_pf(D), D)
    def d_pf(d):
        return num1(np.interp(d, D, differential_array))
    return d_pf


def differential2cumulative_pf(
    d_pf, dmin=0.005, dmax=10**3.51, n_points=10000
):
    D = np.logspace(np.log10(dmin), np.log10(dmax), n_points)
    fD = np.flip(D)
    ...
    def c_pf(d):
        return num1(np.interp(d, D, cumulative_array))
    return c_pf


def differential2R_pf(d_pf):
    def R_pf(d):
        return d_pf(d) * d**3
    return R_pf
    

def R2differential_pf(R_pf):
    def d_pf(d):
        return R_pf(d) / d**3
    return d_pf
...
def R2cumulative_pf(
    R_pf, dmin=0.005, dmax=10**3.51, n_points=10000
):
    d_pf = R2differential_pf(R_pf)
    args = match_args(locals(), differential2cumulative_pf)
    return differential2cumulative_pf(**args)


def cumulative2R_pf(
    c_pf, dmin=0.005, dmax=10**3.51, n_points=10000
):
    args = match_args(locals(), cumulative2differential_pf)
    d_pf = cumulative2differential_pf(**args)
    return differential2R_pf(d_pf)
...
def linear_pf(N1=0.001, slope=-2):
    def out_f(d):
        return num1(10**(slope * np.log10(d) + np.log10(N1)))
    return out_f
...
def linear_pf_R(alpha=2, D=1, R=0.1):
    def out_f(d):
        ...
        x1 = np.log10(D)
        y1 = np.log10(R)
        return num1(10**(m * (x - x1) + y1))
    return out_f


def loglog_linear_pf(N1=0.001, slope=-2):
    def out_f(logd):
        return num1(slope * logd + np.log10(N1))
    return out_f
...
def polynomial_pf(D, coefficients):
    ...
    return num1(10**np.sum(a_n * logD_matrix**n, axis=1))
...
def hartmann84_sat(D):
    return num1(10**(-1.83 * np.log10(D) - 1.33))


def hartmann84_sat_D(age, Ds, pf=npf_new_loglog):
    d = np.flip(Ds)
    diff = np.log10(age) + pf(np.log10(d)) - np.log10(hartmann84_sat(d))
    return num1(np.interp(0, diff, d))


# The Neukum Chronology Function.
def ncf(t):
    return num1(5.44E-14 *(np.exp(6.93 * t) - 1) + 8.38E-4 * t)

def dncf_dt(t):
    return num1(5.44E-14 * 6.93 * np.exp(6.93 * t) + 8.38E-4)
...
class ncf_model():
    def __init__(self, nseg=10000):
        ...
        self.N1 = ncf(self.T)
    def inv(self, N1):
        return num1(np.interp(N1, self.N1, self.T))
...
def ncf_mars(t):
    return num1(2.68E-14 *(np.exp(6.93 * t) - 1) + 4.13E-4 * t)
...
def synth_pf(d_km):
    return num1(10**(-3 - 2*np.log10(d_km)))
